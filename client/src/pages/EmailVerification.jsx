...
const EmailVerification = ({ setActivePage }) => {
    ...
    const formatIssuesCount = verificationResults.filter(
        (result) => result.status === 'Format Issue'
    ).length;
    const undeliverableCount = verificationResults.filter(
        (result) => result.status === 'Undeliverable'
    ).length;
        ...
        <div className="email-verification-page">
            <div className="page-header">
                <div className="breadcrumb">
                    ...
                    <span>›</span>
                    <a href="#" onClick={() => setActivePage('data-cleaning')}>
                        Data Cleaning
                    </a>
                    <span>›</span>
                ...
                <p className="verification-summary">
                    Verification completed for {verificationResults.length} leads
                </p>
                            ...
                            {filteredResults.length > 0 ? (
                                filteredResults.map((result) => (
                                    <tr key={result.id || result.lead_id}>
                                        ...
                                        <td
                                            className={`result-status ${(result.status || '').toLowerCase().replace(' ', '-')}`}
                                        >
