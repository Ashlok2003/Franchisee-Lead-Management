...
const Promotion = ({ setActivePage }) => {
    ...
    const [whatsappMessage, setWhatsappMessage] = useState(
        'Hello [name]! we have an exciting franchise opportunity for [business_type] in your area. Would you be interested in learning more?'
    );

    // Email content state
    const [emailSubject, setEmailSubject] = useState(
        'Exclusive Franchise Opportunity for [Business_type]'
    );
...
- Next steps in the application process`
    ...
    const messagesTracking = [
        { id: 1, name: 'John Doe', time: 'June 7, 10:45 AM', avatar: '/api/placeholder/40/40' },
        {
            id: 2,
            name: 'Sarah Johnson',
            time: 'February 12, 10:30 AM',
            avatar: '/api/placeholder/40/40',
        },
        {
            id: 3,
            name: 'Michael Brown',
            time: 'March 7, 10:15 AM',
            avatar: '/api/placeholder/40/40',
        },
    ];
    ...
    const [templates, setTemplates] = useState([
        ...
        },
        {
            ...
            content:
                'Hi [name], just following up on our previous conversation about the [business_type] franchise opportunity. Have you had a chance to review the information?',
        },
        {
            id: 3,
            name: 'Special Promotion',
            content:
                'Exclusive offer for [name]: Sign up for our [business_type] franchise before [date] and receive 15% off the initial franchise fee!',
        },
    ]);
    ...
    const generateEmailContent = (prompt) => {
        ...
        if (
            prompt.toLowerCase().includes('promotional') ||
            prompt.toLowerCase().includes('promotion')
        ) {
...
[Email]`;
    ...
    const generateCallScript = (prompt) => {
        if (
            ...
            prompt.toLowerCase().includes('initial')
        ) {
    ...
    return (
        <div className="promotions-container">
            ...
            <p className="promotions-subtitle">
                Manage outreach campaigns, track communication, and update lead status
            </p>
                            ...
                            <tbody>
                                {filteredLeads.map((lead) => (
                                    <tr key={lead.id}>
                                        ...
                                        <td>
                                            <span
                                                className={`status-tag ${lead.status.toLowerCase().replace(' ', '-')}`}
                                            >
                                                {lead.status}
                                            </span>
                                        </td>
                ...
                {selectedAction === 'Email' && (
                    <>
                        ...
                        <div className="email-actions">
                            <button className="send-now-btn" onClick={sendMessageNow}>
                                Send Now
                            </button>
                            <button
                                className="schedule-btn"
                        ...
                        <div className="modal-body">
                            <div className="ai-prompt-form">
                                <div className="form-group">
                                    <label>
                                        Prompt for{' '}
                                        {selectedAction === 'Email' ? 'Email' : 'Message'}{' '}
                                        Generation
                                    </label>
                                    <textarea
                                        value={aiPrompt}
                            ...
                            <div className="generation-info">
                                <p>
                                    <strong>Note:</strong> The generated content will appear
                                    directly in your{' '}
                                    {selectedAction === 'Email' ? 'email body' : 'message'} when
                                    ready.
                                </p>
                            </div>
                        </div>
