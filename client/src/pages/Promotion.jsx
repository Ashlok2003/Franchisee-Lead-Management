...
- Next steps in the application process`
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        setLoading(true);
        // The endpoint matches the one we defined in the backend
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      ...
      const filtered = leads.filter(lead => 
        ...
        (lead.TypeOfLead && lead.TypeOfLead.toLowerCase().includes(query))
      );
  ...
  const handleAddRecipients = () => {
    ...
    const unselectedLeads = leads.filter(lead => !selectedLeadsIds.includes(lead.id));
    setAvailableLeads(unselectedLeads);
  ...
  // Function to add a lead from the modal
  const addLeadFromModal = (lead) => {
    if (!selectedLeadsIds.includes(lead.id)) {
      // Add to selected leads IDs
      setSelectedLeadsIds(prev => [...prev, lead.id]);
      
      // Add to selected leads with appropriate info based on action type
      const leadInfo = {
        id: lead.id,
        name: lead.name,
        email: lead.email,
      ...
      setSelectedLeads(prev => [...prev, leadInfo]);
    }
  };
      ...
      <div className="section-container">
        ...
        <div className="search-filter-container">
          <div className="search-box">
            ...
            <input 
              type="text" 
              placeholder="Search by name, location, phone, email, business type, source, status..." 
              value={searchQuery}
        ...
        {selectedAction === "WhatsApp Business" && (
          <>
            <div className="selected-leads-list">
              ...
              <button className="add-recipients-btn" onClick={handleAddRecipients}>
                <Plus size={16} />
                Add More Recipients
              </button>
            </div>
