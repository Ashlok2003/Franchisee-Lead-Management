    ...
    useEffect(() => {
        const fetchLeads = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:5000/api/cleaning?status=${filter}`);
        ...
        <div className="data-cleaning-page">
            ...
            <div className="search-container">
                ...
                <button
                    ...
                    onClick={() => setActivePage('leads-management')}
                >
                    Back to Leads
                </button>
            </div>
            ...
            <div className="tabs">
                <button
                    className={`tab ${activeCleaningTab === 'all' ? 'active' : ''}`}
                    onClick={() => setActiveCleaningTab('all')}
                >
                    All Leads
                </button>
                <button
                    className={`tab ${activeCleaningTab === 'needs-verification' ? 'active' : ''}`}
                    onClick={() => setActiveCleaningTab('needs-verification')}
                >
                    Needs Verification
                </button>
                <button
                    className={`tab ${activeCleaningTab === 'verified' ? 'active' : ''}`}
                    onClick={() => setActiveCleaningTab('verified')}
                >
                    Verified
                </button>
                <button
                    className={`tab ${activeCleaningTab === 'invalid-data' ? 'active' : ''}`}
                    onClick={() => setActiveCleaningTab('invalid-data')}
                >
                    Invalid Data
                </button>
            </div>
            ...
            <div className="leads-header">
                <div className="leads-count">
                    <span>
                        ...
                        {activeCleaningTab !== 'all'
                            ? statusLabels[activeCleaningTab] || 'Leads'
                            : 'Leads'}
                    </span>
                </div>

                <div className="leads-actions">
                    <button className="btn btn-primary" onClick={handleBulkVerification}>
                        Bulk Verify
                    </button>
                    ...
                    <table className="leads-table">
                        ...
                        <tbody>
                            {filteredLeads.length > 0 ? (
                                filteredLeads.map((lead) => (
                                    <tr key={lead.id}>
                                        ...
                                        <td className="lead-contact">
                                            {lead.email_id || 'No email'} <br />{' '}
                                            {lead.contact_number || 'No phone'}
                                        </td>
                                        <td className="lead-interest">
                                        ...
                                        <td className="lead-actions">
                                            <button
                                                ...
                                                onClick={() => handleEmailVerification(lead)}
                                                title="Verify Email"
                                            >
                                            ...
                                            <button
                                                className="action-btn"
                                                onClick={() =>
                                                    updateCleaningStatus(lead.id, 'Invalid-Data')
                                                }
                                ...
                                <tr>
                                    <td colSpan="6" className="no-leads-message">
