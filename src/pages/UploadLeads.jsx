...
const UploadLeads = () => {
  ...
  // Update the fetchUploadHistory function
const fetchUploadHistory = async () => {
  try {
    const response = await axios.get("http://localhost:5000/api/leads/history");
    console.log("Fetched history from backend:", response.data);

    if (response.data && Array.isArray(response.data)) {
      // Filter by leadType
      const existing = response.data.filter(item => item.leadType === 'Existing');
      const newLeads = response.data.filter(item => item.leadType === 'New');
    ...
    // If fetch fails, we still have localStorage data
  }
};
  ...
  const downloadCSV = () => {
  console.log("Downloading template from: http://localhost:5000/api/leads/template");
  
  // You can use fetch to better handle errors
  fetch("http://localhost:5000/api/leads/template")
    .then(response => {
      if (!response.ok) {
  ...
  const downloadUpload = (id) => {
    // For debugging, log the download URL
    console.log(`Downloading file: http://localhost:5000/api/leads/download/${id}`);
      ...
      {activeTab === 'existing' && (
        <div className="upload-section">
          ...
          <button 
            ...
            onClick={downloadCSV} 
            title="C:\Users\Kamini Harad\franchisee\Lead Template.xlsx"
          >
            Download Template
          </button>
